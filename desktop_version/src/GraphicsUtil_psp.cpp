...
Uint32 GraphicsStats::BSS_time = 0;
Uint32 GraphicsStats::BSC_time = 0;
Uint32 GraphicsStats::BSS_numcalls = 0;
Uint32 GraphicsStats::BSC_numcalls = 0;
...
SDL_Surface* GetSubSurface( SDL_Surface* metaSurface, int x, int y, int width, int height )
{
    ...
    SDL_Surface* preSurface = SDL_CreateRGBSurface(SDL_SWSURFACE, width, height, 16, r, g, b, a);
...
void BlitSurfaceColoured(
    ...
    Uint16 g = (ct.colour & 0x07E0) >> 5;
    Uint16 b = (ct.colour & 0x001F) >> 0;
    Uint16 a = 0x00;
    Uint16 col16 = ((r >> 0) | (g & 0xFF) << 5 | ((b & 0xFF) << 11));
    ...
    for(int x = 0; x < tempsurface->w; x++)
    {
        for(int y = 0; y < tempsurface->h; y++)
        {
          if (fmt.BitsPerPixel == 16)        
          {
            ...
            // 0x0000 is a generic key
            // 0x2210 is a fonts key
...
{
    SDL_SetColorKey(_src, SDL_SRCCOLORKEY, _key);
    SDL_SetColorKey(_dest, SDL_SRCCOLORKEY, _key);
    SDL_BlitSurface( _src, NULL, _dest, NULL );
    return;
